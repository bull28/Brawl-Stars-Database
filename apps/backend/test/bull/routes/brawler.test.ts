...
describe("Brawlers and Skins endpoints", function(){
    it("/brawler", async function(){
        const res = await request.execute(server).get("/brawler");
    ...
    describe("/brawler/:brawler", function(){
        it("Brawler exists", async function(){
            const res = await request.execute(server).get("/brawler/bull");
            expect(res).to.have.status(200);
        });

        it("Brawler does not exist", async function(){
            const res = await request.execute(server).get("/brawler/not-a-brawler");
    ...
    describe("/skin/:brawler/:skin", function(){
        it("Brawler and skin exist", async function(){
            const res = await request.execute(server).get(`/skin/bull/bullguydefault`);
            expect(res).to.have.status(200);
        });

        it("Brawler exists but not skin", async function(){
            const res = await request.execute(server).get("/skin/bull/not-a-bull-skin");
            expect(res).to.have.status(404);
            expect(res.text).to.equal("Skin not found.");
        });

        it("Both brawler and skin do not exist", async function(){
            const res = await request.execute(server).get("/skin/not-a-brawler/not-a-skin");
            expect(res).to.have.status(404);
    ...
    it("/models", async function(){
        const res = await request.execute(server).get("/models");
        expect(res).to.have.status(200);
        expect(res.body).to.be.an("array");
        expect(res.body).to.have.lengthOf(allSkins.length);
    ...
    it("/skingroups", async function(){
        const res = await request.execute(server).get("/skingroups");
        expect(res).to.have.status(200);
        expect(res.body).to.be.an("array");
    });

    it("/skinfoundin", async function(){
        const res = await request.execute(server).get("/skinfoundin");
        expect(res).to.have.status(200);
        expect(res.body).to.be.an("array");
    });
    ...
    describe("/skinsearch", function(){
        it("Valid search", async function(){
            const res = await request.execute(server).post("/skinsearch").send({filters: {}});
            expect(res).to.have.status(200);
        ...
        it("Too many skin groups", async function(){
            const res = await request.execute(server).post("/skinsearch")
