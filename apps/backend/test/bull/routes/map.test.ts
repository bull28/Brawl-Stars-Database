...
describe("Game Modes and Maps endpoints", function(){
    it("/gamemode", async function(){
        const res = await request.execute(server).get("/gamemode");
    ...
    describe("/gamemode/:gamemode", function(){
        it("Game mode exists", async function(){
            const res = await request.execute(server).get("/gamemode/brawlball");
            expect(res).to.have.status(200);
        });

        it("Game mode does not exist", async function(){
            const res = await request.execute(server).get("/gamemode/not-a-gamemode");
    ...
    describe("/map/:map", function(){
        it("Map exists", async function(){
            const res = await request.execute(server).get(`/map/${events[0].gameModes[0].maps[0].name}`);
            expect(res).to.have.status(200);
        });

        it("Map does not exist", async function(){
            const res = await request.execute(server).get("/map/not-a-map");
            expect(res).to.have.status(404);
