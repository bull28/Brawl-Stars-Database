...
describe("Game Modes and Maps endpoints", function(){
    it("/events/current", async function(){
        const res = await request.execute(server).get("/events/current");
    ...
    describe("/events/worldtime", function(){
        it("Valid world time query", async function(){
            const res = await request.execute(server).get("/events/worldtime").query({second: 1700000000000});
            expect(res).to.have.status(200);
        });

        it("Invalid input", async function(){
            const res = await request.execute(server).get("/events/worldtime").query({second: true});
            expect(res).to.have.status(400);
            expect(res.body).to.eql(createError("EventsInvalidWorldTime"));
        });

        it("Missing entire query", async function(){
            const res = await request.execute(server).get("/events/worldtime");
            expect(res).to.have.status(400);
            expect(res.body).to.eql(createError("EventsInvalidWorldTime"));
        });
    });
    ...
    it("/events/bull", async function(){
        const res = await request.execute(server).get("/events/bull").query({hour: 0, minute: 0, second: 0});
        expect(res).to.have.status(400);
        expect(res.body).to.eql(createError("EventsInvalidSetting"));
    })

    it("/gamemodes", async function(){
        const res = await request.execute(server).get("/gamemodes");
        expect(res).to.have.status(200);
        expect(res.body).to.be.an("object");
    ...
    describe("/gamemodes/:gamemode", function(){
        it("Game mode exists", async function(){
            const res = await request.execute(server).get("/gamemodes/brawlball");
            expect(res).to.have.status(200);
        });

        it("Game mode does not exist", async function(){
            const res = await request.execute(server).get("/gamemodes/not-a-gamemode");
    ...
    describe("/maps/:map", function(){
        it("Map exists", async function(){
            const res = await request.execute(server).get(`/maps/${events[0].rotation[0].name}`);
            expect(res).to.have.status(200);
        });

        it("Map does not exist", async function(){
            const res = await request.execute(server).get("/maps/not-a-map");
            expect(res).to.have.status(404);
    ...
    describe("/mapsearch", function(){
        it("Valid search query", async function(){
            const res = await request.execute(server).get("/mapsearch").query({"search": "a"});
            expect(res).to.have.status(200);
            expect(res.body).to.be.an("object");
        ...
        it("No search query provided", async function(){
            const res = await request.execute(server).get("/mapsearch");
            expect(res).to.have.status(200);
            expect(res.body).to.be.an("object");
            expect(res.body.results).to.be.an("array");
